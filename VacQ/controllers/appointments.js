...
exports.getAppointment=async (req, res, next)=>{
    try{
        ...
        if(!appointment){
            return res.status(400).json({success:false, messgae:`No appointment with the id of ${req.params.id}`});
        }
...
//@route POST /api/v1/hospitals/:hospitalsId/appointments/
//@access Private
exports.addAppointment=async(req, res, next)=>{
    try{
        ...
        if(!hospital){
            return res.status(404).json({success:false, message:`No hospital with the if od ${req.params.hospitalId}`});
        }
...
exports.updateAppointment=async (req, res, next)=>{
    try{
        ...
        if(!appointment){
            return res.status(404).json({success:false.valueOf, message:`No appt with id ${req.params.id}`});
        }
